...
//        $cacheNavKey = $site['username'].'-'.$operationModel::MENU_MANAGE.'-'.$cont_name.'-'.$act_name;
        $cacheMenuKey = $site['username'].'-'.$site['id'].'-'.$cont_name.'-'. $act_name;
        ...
        $operationModel->manageMenu($belong,$site,$cont_name, $act_name);
        // 侧边菜单
        $menuList = Cache::remember(
            $cacheMenuKey,
            function()use($belong,$operationModel,$cont_name ,$act_name,$site){
                return $operationModel->manageMenu($belong,$site,$cont_name, $act_name);
            },
            3600
        );
        ...
        //获取用户的权限信息
        $this->siteId = $siteId = $site['site_id'] ? $site['site_id'] : 0;
        $this->mallId = $mallId = $site['mall_id'] ? $site['mall_id'] : 0;
        $isRoot = $site['is_root'] ? $site['is_root'] : 0;
