...
class Operation extends Common
{
    ...
    {
        return isset($this->parent_id) ? $this->parent_id : 1;
    }
    /**
     * 返回导航信息
     * $param $belong
     ...
     */
    public function nav($belong,$moduleId, $controllerName, $actionName)
    {
     ...
     * @return Operation
     */
    public function getCacheInsForId($id){
        $cacheOperationIns = 'cacheOperationIns-'.$id;
         return Cache::remember(
             $cacheOperationIns,
            function() use($id){
                return (new Operation())->where(array('id' => $id))->find();
            },
            3600
        );
    }
     ...
     * @throws \think\exception\DbException
     */
    public function manageMenu($belong, $user, $controllerName, $actionName)
    {
        // 查询顶级id
        $parent_menu_id = $this->where(['parent_id' => 1,'belong' => $belong])->value('id');

        $isSite = $user['site_id'] ? $user['site_id'] : 0;
        $isMall  = $user['mall_id'] ? $user['mall_id'] : 0;
        $isRoot = $user['is_root'] ? $user['is_root'] : 0;

        //1超级管理员 2区域管理员 3店铺管理员
        $authType = 0;
        if(!empty($isSite)){
            $authType = self::AUTH_SITE;
        }elseif (!empty($isMall)){
            $authType = self::AUTH_MALL;
        }elseif (!empty($isRoot)){
            $authType = self::AUTH_ROOT;
        }
        ...
        $list = $this
            ->distinct(true)
            ->where('belong',$belong)
            ->where('perm_type',self::PERM_TYPE_SUB)
            ->order('sort desc')
            ->select();
     ...
     * @throws \think\exception\DbException
     */
    public function manageMenuOld($manage_id, $controllerName, $actionName)
    {
...
//    }
    ...
    public function toAdd($data){
        ...
        if(!isset($data['parent_id']) || !isset($data['name']) || !isset($data['code']) || !isset($data['type']) || !isset($data['perm_type'])){
