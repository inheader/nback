...
class OrderLog extends Common
{
    ...
    public function addData(array $info){
        ...
        //         'orderSn'        => 'required',
        //         'logMsg'         => 'required',
        //         'logOrderState'  => 'required',
        //         'createPerson'   => 'required'
        //     ]);
        // }catch(ValidationException $validateException){
        //     throw new ModelException('添加订单日志失败', GlobalErrCode::ERR_PARAM_VALIDATE_ERROR);
        // }

        $data = array();
        $data['mall_id']        = $info['mallId'];
        $data['order_id']       = $info['orderId'];
        $data['order_sn']       = $info['orderSn'];
        $data['log_msg']        = trim($info['logMsg']);
        $data['log_time']       = time();
        $data['log_orderstate'] = $info['logOrderState'];
        $data['create_person']  = $info['createPerson'];
     ...
     * @throws \think\exception\DbException
     */
    public function getOrderLogListForOrderId($orderId)
    {
        $logInsList = $this->where('order_id',$orderId)
            ->order('log_time DESC')->select();
        return collect($logInsList)->map(function($ins){
            return[
                'order_sn'  =>$ins['order_sn'],
                'log_msg'   =>$ins['log_msg'],
                'log_time'  =>date("Y-m-d H:i:s",$ins['log_time']),
            ];
        })->all();
    }
     ...
     * @return int|string
     */
    public function addOrderLog($mallId,$orderId,$orderSn,$logMsg,$orderState)
    {
        $info['mall_id']        = $mallId;
        $info['order_id']       = $orderId;
        $info['order_sn']       = $orderSn;
        $info['log_msg']        = $logMsg;
        $info['log_orderstate'] = $orderState;
        $info['log_time'] = time();
    ...
    /**
     * @param OrderCommon $ordIns
     * @param $msg
     * @param $who
     */
    public function addOrderActLog(OrderCommon $ordIns,$msg,$who) {
        $orderLogIns['mall_id']        = $ordIns->getMyMallId();
        $orderLogIns['order_id']       = $ordIns->getMyId();
        $orderLogIns['order_sn']       = $ordIns->getMyOrderSN();
        $orderLogIns['log_msg']        = trim($msg);
        $orderLogIns['log_time']       = time();
        $orderLogIns['log_orderstate'] = $ordIns->getMyOrderState();
        $orderLogIns['create_person']  = $who;
        $data['updated_at'] = date('Y-m-d H:i:s');
        $this->insert($orderLogIns);
    }
}