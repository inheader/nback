...
class GoodsCat extends Common
{
     ...
     */
    public function getList($userWhere=[])
    {
        $filter = [];
        if(!empty($userWhere)){
     ...
     * @throws \think\exception\DbException
     */
    public function getAllCat( $id = false,$userWhere=[])
    {
        $where = [];
        if($id)
        {
...
//            $where = array_merge($where,$userWhere);
     ...
     * @throws \think\exception\DbException
     */
    public function getFirstCatList($userWhere=[])
    {
        //一级分类的子ID是0
        $where = [
            'parent_id'=>0
     ...
     * @throws \think\exception\DbException
     */
    public function getCatByParentId($parentId = 0,$userWhere=[])
    {
