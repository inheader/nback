...
class MallManage extends Common
{
...
//        }
     ...
     */
    public function getUserNameForMallId($mallId){
        $userList = $this
            ->where('mall_id',$mallId)
            ->where('mall_id','<>',0)
            ->select();
        return collect($userList)->map(function($userInfo){
            return [
     ...
     * @throws \think\exception\DbException
     */
    public function getUserNameForSiteId($siteId){

        $userList = $this->where('site_id',$siteId)->select();
     ...
     * @throws \think\exception\DbException
     */
    public function getInsForUserName($userNames){
        $userInfo = $this->whereIn('username',$userNames)->find();
     ...
     * @throws \think\exception\DbException
     */
    public function addMallUser($mallId,$userName,$passWord){
        //更新账号信息
        $userIns = $this->where('username',$userName)->find();
        if(empty($userIns)){
            $time = time();
            $password = $this->enPassword($passWord,$time);
     ...
     * @throws \think\exception\DbException
     */
    public function addSiteUser($siteId,$userName,$passWord){
        //更新账号信息
        $userIns = $this->where('username',$userName)->find();
        if(empty($userIns)){
            $time = time();
            $password = $this->enPassword($passWord,$time);
            $saveInfo['site_id'] = $siteId;
            $saveInfo['username'] = $userName;
            $saveInfo['ctime'] = $time;
            $saveInfo['password'] = $password;
            $this->insert($saveInfo);
