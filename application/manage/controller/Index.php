...
class Index extends Manage
{
    public function index()
    {
        ...
        $unpaid_count = $orderModel->where($userWhere)
            ->where('order_state', OrderCommon::PENDING_PAYMENT)
            ->count();

        //待发货数量
        $orderModel = new OrderCommon();
        $unship_count = $orderModel->where($userWhere)
            ->where('order_state', OrderCommon::PENDING_DELIVERY)
            ->count();

        //待售后数量
        $orderReturnModel = new OrderReturn();
        $afterSales_count = $orderReturnModel->where($userWhere)
            ->where('status','in',[1,6])
            ->count();
