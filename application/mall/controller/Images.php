...
class Images extends Mall
{
    ...
    function uploadImage()
    {
        ...
        $image_extensions = explode(',','jpg,jpeg,png,gif,bmp4,pdf');
        if(Request::isPost()) {
            ...
            if ($image_storage && $image_storage['type'] == 'qiniu') {
                //$file_extension   = get_file_extension($_FILES['upfile']['name']);
                $save_path = '';
            }elseif(isset($_FILES['upfile'])){
                //$file_extension   = get_file_extension($_FILES['upfile']['name']);
                $save_path = DIRECTORY_SEPARATOR . 'static' . DIRECTORY_SEPARATOR . 'uploads' . get_hash_dir($_FILES['upfile']['name']);
            }else{
                //$file_extension   = get_file_extension($_FILES['file']['name']);
                $save_path = DIRECTORY_SEPARATOR . 'static' . DIRECTORY_SEPARATOR . 'uploads' . get_hash_dir($_FILES['file']['name']);
...
//            }
            ...
            if($info) {
                ...
                $url           = getRealUrl($save_path . $first['savename']);
                $preview_url   = $url;
