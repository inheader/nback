...
class Goods extends Site
{
    ...
    private function checkGoodsInfo($isEdit = false)
    {
        ...
        //平台会员设置
        $data['goods']['mall_member_price']            = input('post.goods.mall_member_price', 0);
        //平台会员价格设置判断 如果平台价格大于销售价 提示异常
        if($data['goods']['mall_member_price'] > $data['goods']['price'])
        {
            $result = [
                'status' => false,
                'msg'    => '超出销售价格设置',
        ...
        $data['goods']['mall_member_discount']         = input('post.goods.mall_member_discount', 0.001);
        //平台销售分佣
        $data['goods']['mall_agent_discount']         = input('post.goods.mall_agent_discount', 0.001);
        if($data['goods']['mall_agent_discount'])
        {
            $data['goods']['mall_agent_price']        = sprintf('%.2f',$data['goods']['mall_agent_discount'] * $data['goods']['mall_member_price']);
        }
        //平台分佣
        $data['goods']['mall_commission_discount']         = input('post.goods.mall_commission_discount', 0.001);
        if($data['goods']['mall_commission_discount'])
        {
            $data['goods']['mall_commission_price']        = sprintf('%.2f',$data['goods']['mall_commission_discount'] * $data['goods']['mall_member_price']);
        }
        //销售分佣
        $data['goods']['agent_discount']         = input('post.goods.agent_discount', 0.001);
        if($data['goods']['agent_discount'])
        {
            $data['goods']['agent_price']         = sprintf('%.2f',$data['goods']['agent_discount'] * $data['goods']['price']);
        }
        $data['goods']['agent_discount1']         = input('post.goods.agent_discount1', 0.001);
        if($data['goods']['agent_discount1'])
        {
            $data['goods']['agent_price1']         = sprintf('%.2f',$data['goods']['agent_discount1'] * $data['goods']['price1']);
        }
        $data['goods']['agent_discount2']         = input('post.goods.agent_discount2', 0.001);
        if($data['goods']['agent_discount2'])
        {
            $data['goods']['agent_price2']         = sprintf('%.2f',$data['goods']['agent_discount2'] * $data['goods']['price2']);
        }
        $data['goods']['agent_discount3']         = input('post.goods.agent_discount3', 0.001);
        if($data['goods']['agent_discount3'])
        {
            $data['goods']['agent_price3']         = sprintf('%.2f',$data['goods']['agent_discount3'] * $data['goods']['price3']);
        }
        $data['goods']['agent_discount4']         = input('post.goods.agent_discount4', 0.001);
        if($data['goods']['agent_discount4'])
        {
            $data['goods']['agent_price4']         = sprintf('%.2f',$data['goods']['agent_discount4'] * $data['goods']['price4']);
        }
        $data['goods']['agent_discount5']         = input('post.goods.agent_discount5', 0.001);
        if($data['goods']['agent_discount5'])
        {
            $data['goods']['agent_price5']         = sprintf('%.2f',$data['goods']['agent_discount5'] * $data['goods']['price5']);
        }
        $data['goods']['agent_discount6']         = input('post.goods.agent_discount6', 0.001);
        if($data['goods']['agent_discount6'])
        {
            $data['goods']['agent_price6']         = sprintf('%.2f',$data['goods']['agent_discount6'] * $data['goods']['price6']);
        }

        //分佣
        $data['goods']['commission_discount']         = input('post.goods.commission_discount', 0.001);
        if($data['goods']['commission_discount'])
        {
            $data['goods']['commission_price']         = sprintf('%.2f',$data['goods']['commission_discount'] * $data['goods']['price']);
        }
        $data['goods']['commission_discount1']         = input('post.goods.commission_discount1', 0.001);
        if($data['goods']['commission_discount1'])
        {
            $data['goods']['commission_price1']         = sprintf('%.2f',$data['goods']['commission_discount1'] * $data['goods']['price1']);
        }
        $data['goods']['commission_discount2']         = input('post.goods.commission_discount2', 0.001);
        if($data['goods']['commission_discount2'])
        {
            $data['goods']['commission_price2']         = sprintf('%.2f',$data['goods']['commission_discount2'] * $data['goods']['price2']);
        }
        $data['goods']['commission_discount3']         = input('post.goods.commission_discount3', 0.001);
        if($data['goods']['commission_discount3'])
        {
            $data['goods']['commission_price3']         = sprintf('%.2f',$data['goods']['commission_discount3'] * $data['goods']['price3']);
        }
        $data['goods']['commission_discount4']         = input('post.goods.commission_discount4', 0.001);
        if($data['goods']['commission_discount4'])
        {
            $data['goods']['commission_price4']         = sprintf('%.2f',$data['goods']['commission_discount4'] * $data['goods']['price4']);
        }
        $data['goods']['commission_discount5']         = input('post.goods.commission_discount5', 0.001);
        if($data['goods']['commission_discount5'])
        {
            $data['goods']['commission_price5']         = sprintf('%.2f',$data['goods']['commission_discount5'] * $data['goods']['price5']);
        }
        $data['goods']['commission_discount6']         = input('post.goods.commission_discount6', 0.001);
        if($data['goods']['commission_discount6'])
        {
            $data['goods']['commission_price6']         = sprintf('%.2f',$data['goods']['commission_discount6'] * $data['goods']['price6']);
        }

        //其他
        $data['goods']['volume']        = input('post.goods.volume', '');
    ...
    private function checkProductInfo($data, $goods_id = 0, $isEdit = false)
    {
        ...
        //平台会员设置
        $data['product']['mall_member_price']            = $data['goods']['mall_member_price'];
        $data['product']['mall_member_discount']         = $data['goods']['mall_member_discount'];
        //平台销售分佣
        $data['product']['mall_agent_discount']         = $data['goods']['mall_agent_discount'];
        if($data['product']['mall_agent_discount'])
        {
            $data['product']['mall_agent_price']        = sprintf('%.2f',$data['product']['mall_agent_discount'] * $data['product']['mall_member_price']);
        }
        //平台分佣
        $data['product']['mall_commission_discount']         = $data['goods']['mall_commission_discount'];
        if($data['product']['mall_commission_discount'])
        {
            $data['product']['mall_commission_price']        = sprintf('%.2f',$data['product']['mall_commission_discount'] * $data['product']['mall_member_price']);
        }

        //销售分佣
        $data['product']['agent_discount']          = $data['goods']['agent_discount'];
        if($data['product']['agent_discount'])
        {
            $data['product']['agent_price']         = sprintf('%.2f',$data['product']['agent_discount'] * $data['product']['price']);
        }
        $data['product']['agent_discount1']         = $data['goods']['agent_discount1'];
        if($data['product']['agent_discount1'])
        {
            $data['product']['agent_price1']        = sprintf('%.2f',$data['product']['agent_discount1'] * $data['product']['price1'] );
        }
        $data['product']['agent_discount2']         = $data['goods']['agent_discount2'];
        if($data['product']['agent_discount2'])
        {
            $data['product']['agent_price2']        = sprintf('%.2f',$data['product']['agent_discount2'] * $data['product']['price2']);
        }
        $data['product']['agent_discount3']         = $data['goods']['agent_discount3'];
        if($data['product']['agent_discount3'])
        {
            $data['product']['agent_price3']         = sprintf('%.2f',$data['product']['agent_discount3'] * $data['product']['price3']);
        }
        $data['product']['agent_discount4']         = $data['goods']['agent_discount4'];
        if($data['product']['agent_discount4'])
        {
            $data['product']['agent_price4']        = sprintf('%.2f',$data['product']['agent_discount4'] * $data['product']['price4']);
        }
        $data['product']['agent_discount5']         = $data['goods']['agent_discount5'];
        if($data['product']['agent_discount5'])
        {
            $data['product']['agent_price5']        = sprintf('%.2f',$data['product']['agent_discount5'] * $data['product']['price5']);
        }
        $data['product']['agent_discount6']         = $data['goods']['agent_discount6'];
        if($data['product']['agent_discount6'])
        {
            $data['product']['agent_price6']        = sprintf('%.2f',$data['product']['agent_discount6'] * $data['product']['price6']);
        }

        //分佣
        $data['product']['commission_discount']          = $data['goods']['commission_discount'];
        if($data['product']['commission_discount'])
        {
            $data['product']['commission_price']         = sprintf('%.2f',$data['product']['commission_discount'] * $data['product']['price']);
        }
        $data['product']['commission_discount1']         = $data['goods']['commission_discount1'];
        if($data['product']['commission_discount1'])
        {
            $data['product']['commission_price1']         = sprintf('%.2f',$data['product']['commission_discount1'] * $data['product']['price1']);
        }
        $data['product']['commission_discount2']          = $data['goods']['commission_discount2'];
        if($data['product']['commission_discount2'])
        {
            $data['product']['commission_price2']         = sprintf('%.2f',$data['product']['commission_discount2'] * $data['product']['price2']);
        }
        $data['product']['commission_discount3']         = $data['goods']['commission_discount3'];
        if($data['product']['commission_discount3'])
        {
            $data['product']['commission_price3']         = sprintf('%.2f',$data['product']['commission_discount3'] * $data['product']['price3']);
        }
        $data['product']['commission_discount4']         = $data['goods']['commission_discount4'];
        if($data['product']['commission_discount4'])
        {
            $data['product']['commission_price4']         = sprintf('%.2f',$data['product']['commission_discount4'] * $data['product']['price4']);
        }
        $data['product']['commission_discount5']         = $data['goods']['commission_discount5'];
        if($data['product']['commission_discount5'])
        {
            $data['product']['commission_price5']         = sprintf('%.2f',$data['product']['commission_discount5'] * $data['product']['price5']);
        }
        $data['product']['commission_discount6']         = $data['goods']['commission_discount6'];
        if($data['product']['commission_discount6'])
        {
            $data['product']['commission_price6']         = sprintf('%.2f',$data['product']['commission_discount6'] * $data['product']['price6']);
        }
    ...
    public function doEdit()
    {
        ...
        $updateRes = $goodsModel->updateGoods($data['goods']['id'], $data['goods']);//更新商品
        ...
        if ($open_spec) {
            ...
            foreach ($product as $key => $val) {
                ...
                if (isset($val['id'])&&$val['id']) {
                    $productRes = $productsModel->updateProduct($val['id'], $data['product']);//更新产品
                    ...
                    $productRes = $productsModel->doAdd($data['product']);//添加产品
            ...
            $upData['price5']     = $price5;
            $upData['price6']     = $price6;
            $goodsModel->updateGoods($goods_id, $upData);//更新库存商品
            //删除多余规格
            ...
            if ($data['product']['id']) {
                ...
                $updateRes = $productsModel->updateProduct($data['product']['id'], $data['product']);//更新产品
            } else {
                $updateRes = $productsModel->doAdd($data['product']);//添加产品
            }
